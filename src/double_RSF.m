function [u1, u2] = double_RSF(varargin)
...
param1.KI = conv2(Img, param1.Ksigma, 'same');
param1.KONE = conv2(ones(size(Img)), param1.Ksigma, 'same');
sigma = param2.sigma;
param2.Ksigma = fspecial('gaussian', round(2 * sigma) * 2 + 1, sigma); % the Gaussian kernel
param2.KI = conv2(Img, param2.Ksigma, 'same');
param2.KONE = conv2(ones(size(Img)), param2.Ksigma, 'same');
...
% single step of RSF method with region mutual exclusion term
    ...
    lambda1   = param.lambda1;  % coefficient of the length term
    lambda2   = param.lambda2;  % time step
    nu        = param.nu;       % coefficient of the level set (distance) regularization term P(\phi)
    timestep  = param.timestep; % the papramater in the definition of smoothed Dirac function
    mu        = param.mu;       % Gaussian kernel size
    epsilon   = param.epsilon;  % coefficient of prior term
    alpha     = param.alpha;    % coefficient of mutual exclusion term
    eta       = param.eta;      % draw step, default 0 is not draw
    KI        = param.KI;
    KONE      = param.KONE;
    Ksigma    = param.Ksigma;
